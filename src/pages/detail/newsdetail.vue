...
//       document.documentElement.scrollTop = document.body.scrollTop = 0
  ...
  mounted() {
    ...
    this.$nextTick(() => {
      ...
      this.$root.$on('addcomment', value => {
        this.addNewComment(value)
      })
    })
  },
  beforeDestroy() {
    this.$root.emitter.$off('addcomment')
  },
  destroyed() {
  },
  methods: {
      ...
      articleDetail(this.itemid)
        .then(response => {
          ...
          if (this.$route.query.hasvideo) {
            this.$nextTick(() => {
              // 初始化视频数据
              // 当多次在当前页面跳转时，初始化视频会加载不出来，需要延迟（猜测是由于上一页面的影响，需在上一页面完全销毁后）
              setTimeout(() => {
                this.initXgplayer()
              }, 500)
            })
    ...
    getComment() {
      ...
      comments(params).then(response => {
        ...
        // 对comments进行排序
        data.comments.sort(this.compare('create_time'))
    ...
    addNewComment(e) {
      const comment = {
        ...
        'dongtai_id': '6684124028174712835',
        'user_digg': 0,
        'id': this.randomWord(9)
      }
      this.comjson.comments.push(comment)
      this.comjson.comments.sort(this.compare('create_time'))
    },
    randomWord(count) {
